...
function getBestMimeType() {
  // Android Chrome solo soporta WebM para MediaRecorder
  // Probamos igual MP4 por si algún dispositivo lo soporta
  const types = [
...
function getFileExtension(mimeType) {
  // Si viene de un archivo externo (Grabadora de Samsung, etc.) preservar su extensión real
  if (typeof _externalFileName !== 'undefined' && _externalFileName) {
    const m = _externalFileName.match(/\.([^.]+)$/);
  ...
  // WebM con Opus → renombrar a .ogg para mayor compatibilidad
  // El contenido es idéntico, solo cambia el contenedor reconocido
  return 'ogg';
}
...
async function startRecording() {
  ...
  const quality  = parseInt(document.getElementById('quality-select').value) || 256000;
  const mimeType = getBestMimeType();
  _audioChunks   = [];
  _currentBlob   = null;

  // Bitrate mínimo garantizado para buena calidad
  const bitsPerSec = Math.max(quality, 256000);
  const options = { audioBitsPerSecond: bitsPerSec };
  if (mimeType) options.mimeType = mimeType;

  console.log('Grabando con:', mimeType, bitsPerSec + 'bps');
...
// ── Subir archivo grabado externamente (Grabadora de Samsung, etc.) ──
function handleExternalFile(event) {
  const file = event.target.files && event.target.files[0];
  if (!file) return;

  if (_isRecording) {
    toast('Detené la grabación actual antes de subir un archivo', 'error');
    event.target.value = '';
    return;
  }

  // Usar el blob del archivo directamente — preserva la calidad original
  _currentBlob = file;
  _externalFileName = file.name; // guardar nombre original para sugerir filename

  // Pre-rellenar el nombre del archivo (sin extensión) si está vacío
  const nameInput = document.getElementById('rec-filename');
  ...
  showPreview(_currentBlob);
  toast('Archivo cargado: ' + file.name + ' (' + (file.size/1024/1024).toFixed(1) + ' MB)', 'success');

  event.target.value = ''; // permitir elegir el mismo archivo de nuevo
}

let _externalFileName = null;
