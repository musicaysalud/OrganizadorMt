...
async function cronScreenLoad() {
  ...
  try {
    ...
    const esToken = e.message && e.message.includes('expirado');
    cont.innerHTML = `<div style="padding:20px;color:var(--red);font-size:.82rem;">
      ${esToken
        ? `<div style="font-size:1rem;margin-bottom:8px;">⚠️ Sesión de Google expirada</div>
           <div style="color:var(--text);margin-bottom:16px;">Los tokens duran 1 hora. Reconectá tu cuenta para continuar.</div>
           <button onclick="goScreen('settings')" class="btn-accent" style="width:100%;margin-bottom:8px;">⚙️ Ir a Ajustes para reconectar</button>`
...
async function cronDownloadSync() {
  ...
  if (!_cronSyncFileId) {
    ...
    if (res.status === 401) {
      ...
      _driveToken = null; _tokenExpiry = 0;
      await dbSetSetting(TOKEN_KEY, null).catch(() => {});
      driveUpdateUI(false);
      throw new Error('Token expirado. Reconectá Google Drive en Ajustes.');
    }
