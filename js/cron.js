...
function cronRenderCal(pat) {
  ...
  // emptyCount: cuántas columnas activas hay ANTES del primer día activo del mes
  // Buscar el primer día del mes que caiga en un día activo
  let firstActiveDayOfMonth = 1;
  for (let dd = 1; dd <= 7; dd++) {
    const jsD   = new Date(_cronYear, _cronMonth, dd).getDay();
    const cronD = jsToCol[jsD];
    if (dias.includes(cronD)) { firstActiveDayOfMonth = dd; break; }
  }
  // Las celdas vacías son los días activos de la semana que preceden al primer día activo del mes
  const firstDayCronIdx = jsToCol[new Date(_cronYear, _cronMonth, firstActiveDayOfMonth).getDay()];
  const emptyCount = dias.indexOf(firstDayCronIdx);
  ...
  // Días del mes — solo los que corresponden a un día activo en diasCron
  let col = emptyCount;
  for (let d = 1; d <= daysInMonth; d++) {
    const jsDay   = new Date(_cronYear, _cronMonth, d).getDay(); // 0=Dom..6=Sab
    const cronDay = jsToCol[jsDay]; // convertir a índice L=0..D=6
    if (!dias.includes(cronDay)) continue; // saltar días no activos
    ...
    html += `<div onclick="cronOpenDay('${dateStr}','${pat.id}')"
      style="background:var(--surface);min-height:60px;padding:4px;cursor:pointer;
             ${isToday ? 'border:1px solid var(--accent);' : ''}
             position:relative;">
    ...
    sessions.slice(0,2).forEach(s => {
      html += `<div style="background:${s.color||'var(--accent)'};border-radius:3px;
        padding:2px 4px;font-size:.58rem;color:#fff;margin-bottom:2px;
        overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
        text-shadow:0 1px 2px rgba(0,0,0,.5);">${s.nombre||s.tipo||'Sesión'}</div>`;
    });
