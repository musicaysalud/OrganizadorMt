...
function cronSessForm(dateStr, patId, idx, sess) {
  ...
  dlg.style.cssText = 'position:fixed;inset:0;z-index:2100;background:rgba(0,0,0,.55);'
    + 'display:flex;align-items:flex-end;';

  const [y, m, d] = dateStr.split('-');
  const inp = 'background:var(--surface2);border:1px solid var(--border);color:var(--text);'
    + 'border-radius:10px;padding:14px 16px;font-size:1rem;outline:none;'
    + 'width:100%;box-sizing:border-box;';
  const lbl = (t) => `<div style="font-size:.82rem;color:var(--muted);text-transform:uppercase;`
    + `letter-spacing:.08em;margin-bottom:8px;font-weight:600;">${t}</div>`;
  ...
  dlg.innerHTML = `
    <div style="background:var(--surface);border-radius:20px 20px 0 0;width:100%;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;
                  padding:20px 22px 16px;border-bottom:1px solid var(--border);flex-shrink:0;">
        <div style="font-size:1.15rem;font-weight:700;color:var(--accent);">
          ${idx !== null ? 'Editar' : 'Nueva'} sesión &mdash; ${d}/${m}/${y}
        </div>
      ...
      <div style="overflow-y:auto;flex:1;padding:20px 22px;display:flex;flex-direction:column;gap:20px;">
        <div>
          ${lbl('Actividad')}
          <input id="csf-nombre" type="text" spellcheck="false"
            value="${(sess?.nombre||sess?.tipo||'').replace(/"/g,'&quot;')}"
        ...
        <div>
          ...
          <input id="csf-objetivo" type="text" spellcheck="false"
            value="${(sess?.objetivo||'').replace(/"/g,'&quot;')}"
            placeholder="Objetivo de la sesion..."
            style="${inp}">
        </div>
        <div>
          ${lbl('Observaciones')}
          <textarea id="csf-obs" spellcheck="false" rows="5"
            placeholder="Notas de la sesion..."
            style="${inp}resize:none;">${sess?.obs||''}</textarea>
        </div>
        <div style="display:flex;align-items:center;gap:16px;">
          ${lbl('Color')}
          <input type="color" id="csf-color" value="${sess?.color||'#c8a96e'}"
            style="width:50px;height:50px;border:1px solid var(--border);
                   border-radius:10px;cursor:pointer;padding:3px;flex-shrink:0;">
        </div>
      </div>
      <div style="padding:16px 22px 28px;border-top:1px solid var(--border);
                  flex-shrink:0;display:flex;gap:12px;">
        <button onclick="cronSaveSess('${dateStr}','${patId}',${idx})"
          class="btn-accent" style="flex:1;font-size:1.05rem;padding:16px;">
          &#x1F4BE; Guardar
        </button>
        <button onclick="document.getElementById('cron-form-dlg').remove()"
          class="btn-outline" style="flex:1;font-size:1.05rem;padding:16px;">
          Cancelar
        </button>
      </div>
    </div>`;
