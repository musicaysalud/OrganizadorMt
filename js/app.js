...
async function fpLoadList() {
  ...
  list.innerHTML = '<div id="folder-picker-loading">⟳ Cargando carpetas...</div>';

  const savedFolders = await dbGetFolders();

  // Si estamos navegando dentro de una carpeta, mostrar sus hijos normalmente
  if (_fpCurId) {
    const sub = await driveListFolders(_fpCurId);
    if (!sub.length) {
      list.innerHTML = '<div id="folder-picker-loading" style="color:var(--muted);">Sin subcarpetas.<br><small>Podés seleccionar esta carpeta.</small></div>';
      return;
    }
    list.innerHTML = sub.map(f => {
      const isSaved = savedFolders.some(s => s.driveId === f.id);
      return `<div class="fp-item${isSaved?' saved':''}" onclick="fpNavInto('${f.id}','${f.name.replace(/'/g,"\'").replace(/"/g,'&quot;')}')">
  ...
  // Estamos en la raíz — mostrar Mi unidad + Carpetas del ordenador
  let html = '';

  // 1. Carpetas guardadas como favoritas
  if (savedFolders.length) {
    html += '<div style="padding:8px 18px 4px;font-size:.65rem;color:var(--accent);text-transform:uppercase;letter-spacing:.1em;">★ Favoritas</div>';
  ...
  // 2. Mi unidad
  const myDrive = await driveListFolders(null);
  if (myDrive.length) {
    html += '<div style="padding:8px 18px 4px;font-size:.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:.1em;">📁 Mi unidad</div>';
    html += myDrive.map(f => {
      const isSaved = savedFolders.some(s => s.driveId === f.id);
      return `<div class="fp-item${isSaved?' saved':''}" onclick="fpNavInto('${f.id}','${f.name.replace(/'/g,"\'").replace(/"/g,'&quot;')}')">
        <span class="fp-item-ico">📁</span>
  ...
  // 3. Carpetas sincronizadas del ordenador (búsqueda amplia)
  try {
    const q = `mimeType='application/vnd.google-apps.folder' and trashed=false`;
    const res = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,parents)&orderBy=name&pageSize=200&includeItemsFromAllDrives=true&supportsAllDrives=true&corpora=allDrives`,
      { headers: { Authorization: 'Bearer ' + _driveToken } }
    );
    const data = await res.json();
    const allFolders = data.files || [];

    // IDs de Mi unidad para filtrar
    const myDriveIds = new Set(myDrive.map(f => f.id));

    // Buscar carpetas raíz de computers: las que tienen parents que NO están en myDrive
    // y que no están en myDrive ellas mismas
    const myDriveParents = new Set();
    myDrive.forEach(f => { if (f.parents) f.parents.forEach(p => myDriveParents.add(p)); });

    const computerFolders = allFolders.filter(f => {
      if (myDriveIds.has(f.id)) return false; // ya está en Mi unidad
      if (!f.parents || !f.parents.length) return false;
      // Si su padre no es ninguna carpeta conocida de Mi unidad, es del ordenador
      return !f.parents.some(p => myDriveIds.has(p));
    });

    // Agrupar por padre para mostrar solo las raíces
    const computerRoots = computerFolders.filter(f =>
      f.parents && !computerFolders.some(other => f.parents.includes(other.id))
    );

    if (computerRoots.length) {
      html += '<div style="height:1px;background:var(--border);margin:4px 0;"></div>';
      html += '<div style="padding:8px 18px 4px;font-size:.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:.1em;">🖥 Carpetas del ordenador</div>';
      html += computerRoots.map(f => {
        const isSaved = savedFolders.some(s => s.driveId === f.id);
        return `<div class="fp-item${isSaved?' saved':''}" onclick="fpNavInto('${f.id}','${f.name.replace(/'/g,"\'").replace(/"/g,'&quot;')}')">
          <span class="fp-item-ico">🖥</span>
          <span class="fp-item-name">${f.name}</span>
    ...
    console.error('Error cargando carpetas del ordenador:', e);
  }

  if (!html) {
    html = '<div id="folder-picker-loading" style="color:var(--muted);">No se encontraron carpetas.</div>';
  }
