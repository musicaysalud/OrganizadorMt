...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Sync desde Desktop (?desktop_sync=local) ─────────────────
  const _urlParams   = new URLSearchParams(window.location.search);
  const _desktopSync = _urlParams.get('desktop_sync');
  if (_desktopSync) {
    history.replaceState(null, '', window.location.pathname);
    if (!driveIsConnected()) {
      toast('Conectá Google Drive en Ajustes antes de sincronizar', 'error');
      goScreen('settings');
    } else {
      (async () => {
        // Mostrar panel de diagnóstico visible en pantalla
        const diagDiv = document.createElement('div');
        diagDiv.id = 'sync-diag';
        diagDiv.style.cssText = 'position:fixed;inset:0;z-index:99999;background:#0c0d11;'
          + 'color:#c8a96e;font-family:monospace;font-size:.78rem;padding:20px;overflow-y:auto;';
        diagDiv.innerHTML = '<div style="font-size:1rem;font-weight:bold;margin-bottom:12px;color:#c8a96e;">🔍 Diagnóstico Sync Desktop→Drive</div>'
          + '<div id="diag-log" style="line-height:2;"></div>'
          + '<button onclick="document.getElementById('sync-diag').remove()" '
          + 'style="margin-top:16px;background:#c8a96e;color:#0c0d11;border:none;padding:8px 20px;border-radius:6px;font-size:.85rem;cursor:pointer;">Cerrar</button>';
        document.body.appendChild(diagDiv);

        function dlog(msg, color) {
          const el = document.getElementById('diag-log');
          if (el) el.innerHTML += '<div style="color:' + (color||'#aaa') + '">' + msg + '</div>';
          console.log('[SyncDiag]', msg);
        }

        try {
          dlog('1. Drive conectado: ' + driveIsConnected(), driveIsConnected() ? '#6dc86d' : '#e55');
          dlog('   Token: ' + (_driveToken ? _driveToken.substring(0,20) + '...' : 'NULL'), _driveToken ? '#6dc86d' : '#e55');
          dlog('   Expiry: ' + new Date(_tokenExpiry).toLocaleTimeString(), '#aaa');
          dlog('   Token válido: ' + (Date.now() < _tokenExpiry), Date.now() < _tokenExpiry ? '#6dc86d' : '#e55');

          dlog('2. Buscando datos en Desktop (localhost:47822)...', '#c8a96e');
          let syncData;
          try {
            const dataRes = await fetch('http://localhost:47822/sync-data');
            dlog('   HTTP status: ' + dataRes.status, dataRes.ok ? '#6dc86d' : '#e55');
            if (!dataRes.ok) throw new Error('Desktop no responde (status ' + dataRes.status + ')');
            syncData = await dataRes.json();
            dlog('   Pacientes recibidos: ' + (syncData.pacientes||[]).length, '#6dc86d');
            dlog('   Claves cronograma: ' + Object.keys(syncData.cronograma||{}).length, '#6dc86d');
          } catch(fetchErr) {
            dlog('   ERROR: ' + fetchErr.message, '#e55');
            dlog('   → ¿Está MusiCare Desktop abierto y presionaste ☁?', '#e88');
            throw fetchErr;
          }
          ...
          dlog('   JSON tamaño: ' + json.length + ' bytes', '#aaa');

          dlog('3. Buscando _musicare_sync.json en Drive...', '#c8a96e');
          const q  = "name='_musicare_sync.json' and trashed=false";
          const sr = await fetch(
            'https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(q) + '&fields=files(id,name)',
            { headers: { Authorization: 'Bearer ' + _driveToken } }
          );
          dlog('   Search status: ' + sr.status, sr.ok ? '#6dc86d' : '#e55');
          const sd = await sr.json();
          dlog('   Respuesta Drive: ' + JSON.stringify(sd).substring(0,120), '#aaa');
          const fileId = sd.files && sd.files[0] ? sd.files[0].id : null;
          dlog('   FileId existente: ' + (fileId || 'ninguno — se creará nuevo'), fileId ? '#6dc86d' : '#c8a96e');

          if (fileId) {
            dlog('4. Actualizando archivo existente...', '#c8a96e');
            const upRes = await fetch(
              'https://www.googleapis.com/upload/drive/v3/files/' + fileId + '?uploadType=media',
              { method: 'PATCH', headers: { Authorization: 'Bearer ' + _driveToken, 'Content-Type': 'application/json' }, body: json }
            );
            dlog('   PATCH status: ' + upRes.status, upRes.ok ? '#6dc86d' : '#e55');
            if (!upRes.ok) {
              const errBody = await upRes.text();
              dlog('   Error body: ' + errBody.substring(0,150), '#e55');
              throw new Error('Error actualizando: ' + upRes.status);
            }
            dlog('   ✓ Archivo actualizado', '#6dc86d');
          } else {
            dlog('4. Creando archivo nuevo en Drive...', '#c8a96e');
            const bnd  = 'mcdsync';
            ...
            const cr = await fetch(
              ...
              { method: 'POST', headers: { Authorization: 'Bearer ' + _driveToken, 'Content-Type': 'multipart/related; boundary=' + bnd }, body }
            );
            dlog('   POST status: ' + cr.status, cr.ok ? '#6dc86d' : '#e55');
            const crBody = await cr.text();
            dlog('   Respuesta: ' + crBody.substring(0,150), cr.ok ? '#6dc86d' : '#e55');
            if (!cr.ok) throw new Error('Error creando archivo: ' + cr.status + ' — ' + crBody.substring(0,80));
          }

          dlog('✓ ¡LISTO! _musicare_sync.json escrito en Drive', '#6dc86d');
          dlog('  Pacientes: ' + (syncData.pacientes||[]).length, '#6dc86d');
          toast('✓ Sincronizado con Drive', 'success');
          setTimeout(() => {
            const d = document.getElementById('sync-diag');
            if (d) d.remove();
            goScreen('cron');
          }, 4000);

        } catch(e) {
          dlog('✗ ERROR FINAL: ' + e.message, '#e55');
