...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Manejar sync desde Desktop (?desktop_sync=base64) ──────────
  const urlParams = new URLSearchParams(window.location.search);
  const desktopSync = urlParams.get('desktop_sync');
  if (desktopSync && driveIsConnected()) {
    try {
      const syncData = JSON.parse(atob(desktopSync));
      syncData.lastModified   = new Date().toISOString();
      syncData.lastModifiedBy = 'desktop';
      // Buscar o crear el archivo en Drive usando el token ya existente
      ...
      if (fileId) {
        ...
        const boundary = 'mc_desktop_boundary';
        const body = `--${boundary}
...
--${boundary}--`;
      ...
      toast('✓ Cronograma sincronizado desde Desktop', 'success');
