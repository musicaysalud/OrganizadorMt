...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.location.hash.includes('access_token')) {
    ...
    // Intentar reenviar el token a MusiCare Desktop (Electron) si está corriendo
    let sentToDesktop = false;
    if (token) {
      try {
        const resp = await Promise.race([
          fetch('http://localhost:47821/oauth', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: token, expires_in: expiresIn })
          }),
          new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 1500))
        ]);
        if (resp && resp.ok) {
          sentToDesktop = true;
          // Limpiar URL sin consumir el token para la mobile
          history.replaceState(null, '', window.location.pathname);
          toast('✓ Token enviado a MusiCare Desktop', 'success');
          goScreen('record');
        }
      } catch(e) {
        // Desktop no está corriendo o no respondió — procesar normalmente para mobile
        sentToDesktop = false;
      }
    }
